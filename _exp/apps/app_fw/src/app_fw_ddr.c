...
PRIVATE app_fw_ddr_calibration_data_struct app_fw_ddr_saved_data;
...
PUBLIC PMCFW_ERROR _app_fw_ddr_calibration_save(app_fw_ddr_calibration_data_struct *ddr_training_data);
PUBLIC app_fw_ddr_calibration_save_fn_ptr app_fw_ddr_calibration_save_ptr = _app_fw_ddr_calibration_save;

PRIVATE UINT32 _app_fw_ddr_training_data_addr_get(void);
typedef UINT32 (*app_fw_ddr_training_data_addr_get_fn_ptr)(void);
PRIVATE app_fw_ddr_training_data_addr_get_fn_ptr app_fw_ddr_training_data_addr_get_ptr = _app_fw_ddr_training_data_addr_get;
#define app_fw_ddr_training_data_addr_get (*app_fw_ddr_training_data_addr_get_ptr)
...
PRIVATE PMCFW_ERROR app_fw_ddr_phy_bringup_init(void)
{
    ...
    else
    {
        ...
        if (rc != PMC_SUCCESS)
        {
            bc_printf("ddr_phy_init failed rc = 0x%x\n", rc);
            return rc;
        }
        ...
        app_fw_ddr_calibration_save(&app_fw_ddr_saved_data);
    }
...
PUBLIC PMCFW_ERROR _app_fw_ddr_calibration_save(app_fw_ddr_calibration_data_struct *ddr_training_data)
{
...
#if (APP_FW_TEMP_DISABLE_DDR_SPI == 0)
    ...
    if (PMC_SUCCESS != rc)
    {
        return APP_FW_DDR_ERR_TRAINING_ERASE_TIMEOUT;
    }
    ...
    if (PMC_SUCCESS != rc)
    {
        return APP_FW_DDR_ERR_TRAINING_WRITE;
    }
#else
    rc = PMCFW_ERR_FAIL;
#endif
