...
PRIVATE VOID app_fw_ram_code_ptr_adjust(VOID)
{
    ...
    /* adjust application function pointers */
    app_fw_ddr_calibration_save_ptr = (app_fw_ddr_calibration_save_fn_ptr)((UINT32)app_fw_ddr_calibration_save_ptr - offset);
}
...
void  _34K_VPE0_main(void)
{
    ...
    bc_printf("Booting APP_FW ....\n");

    /* 
    ** I2C must be enabled as soon as possible
    ** HOST will send FW status command followed by BOOT CONFIG command.
    ** BOOT CONFIG will return BUSY until FW is fully up and running.
    */
    spb_mux_init();
