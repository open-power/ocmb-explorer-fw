...
* @note
*/
PRIVATE PMCFW_ERROR flashloader_plat_poll_write_erase_complete(BOOL write, UINT32 timeout)
{
...
PUBLIC UINT32 flashloader_plat_partition_erase(INT8 partition_id, UINT32 *err_code)
{
    ...
    */ 
    ...
    for( i =0; i < flash_subsector_number ; i++, erase_addr+=subsector_len)
    {
        ...
        /* poll for completion */
        rc = flashloader_plat_poll_write_erase_complete(FALSE, flashloader_plat_spi_dev_info.max_time_sector_erase);
        if (rc != PMC_SUCCESS)
        {
            *err_code = rc;
            return FLASHLOADER_ERR_SUBSECTOR_ERASE_TIMEOUT;
        }
    }

    if(write_partition_flag == TRUE)
    {
        rc = spi_flash_write(SPI_FLASH_PORT, SPI_FLASH_CS,
        ...
        /* poll for completion */
        rc = flashloader_plat_poll_write_erase_complete(TRUE,
                                             flashloader_plat_spi_dev_info.max_time_page_prog);
        if (rc != PMC_SUCCESS)
        {
            *err_code = rc;
            return FLASHLOADER_ERR_PARTITION_VALUE_WRITE_TIMEOUT;
        }
    }       
    return PMC_SUCCESS;   
}
...
PUBLIC UINT32 flashloader_plat_flash_buffer_write(UINT32 flash_write_index, UINT8 *flash_image_data_buf, UINT32 *err_code)
{
    ...
    if(!(flash_write_buffer_addr & FLASH_LOADER_SUBSECTOR_BOUNDARY_START))
    {
        ...
        if (rc != PMC_SUCCESS)
        {
            bc_printf("Flashloader: flashloader_plat_flash_buffer_write subsector erase failed\n");
            *err_code = rc;
            return FLASHLOADER_ERR_SUBSECTOR_ERASE;
        }
        ...
        if (rc != PMC_SUCCESS)
        {
            bc_printf("Flashloader: flashloader_plat_flash_buffer_write erase completion timeout\n");
            *err_code = rc;
            return FLASHLOADER_ERR_SUBSECTOR_ERASE_TIMEOUT;
        }
    }
    ...
    rc = spi_flash_write(SPI_FLASH_PORT,
     ...
     rc = flashloader_plat_poll_write_erase_complete(TRUE, flashloader_plat_spi_dev_info.max_time_page_prog);
     if (rc != PMC_SUCCESS)
     {
        *err_code = rc;
        bc_printf("Flashloader: flashloader_plat_flash_buffer_write write completion error: %08lx\n", rc);
        return FLASHLOADER_ERR_FLASH_WRITE_TIMEOUT;
     }
...
/**
* @brief
*   pboot assert function.
*
* @param [in] error_id - Error ID
* @param [in] file - Pointer to char buffer containing file name
* @param [in] line - Line number
* @return
*   None.
*
*/
PUBLIC void pmcfw_assert_function(PMCFW_ERROR error_id,
                                  CHAR        *file,
                                  UINT32      line)
{
    bc_printf("\nBoot Assert !!!\n"
              "error = 0x%x, file = %s, line = %d\n",
              error_id,
              file,
              line);
