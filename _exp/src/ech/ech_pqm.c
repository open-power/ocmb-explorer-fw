...
PRIVATE UINT32 ech_pqm_lane_training(UINT8* rx_buf_ptr, UINT32 rx_index)
{
    ...
    else
    {
        ...
        if (step == 0)
        {
            ...
            /* Initialize SerDes*/
            rc = serdes_plat_low_level_init(ech_pqm_cfg.lanes, ech_pqm_cfg.freq, ech_dfe_state_get());
...
{
    UINT8 mode = rx_buf_ptr[rx_index + EXP_TWI_CMD_DATA_OFFSET + 1];
...
* @note
*/
PRIVATE VOID ech_pqm_prbs_user_defined_pattern_set(UINT8* rx_buf_ptr, UINT32 rx_index)
{
    /* record the PRBS user defined pattern */
    for (UINT32 i= 0; i < ECH_PBRS_USER_PATTERNS; i++)
    {
        ech_pqm_user_prbs.patt[i] = rx_buf_ptr[rx_index + EXP_TWI_CMD_DATA_OFFSET + i];
...
PRIVATE UINT32 ech_pqm_prbs_err_count_start(UINT8* rx_buf_ptr, UINT32 rx_index)
{
    ...
    bc_printf("INFO: Read PRBS error count from lane 0 to lane %d\n",
        ech_pqm_cfg.lanes - 1);
...
PUBLIC VOID ech_pqm_cmd_proc(UINT8* rx_buf_ptr, UINT32 rx_index)
{
    switch (rx_buf_ptr[rx_index])
    {
        ...
        {
            ech_pqm_prbs_user_defined_pattern_set(rx_buf_ptr, rx_index);
        }
        break;
